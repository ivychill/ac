    ...
    def work(self, max_episode_length, gamma, sess, coord, saver):
        ...
        with sess.as_default(), sess.graph.as_default():
            while not coord.should_stop():
                ...
                if episode_count % EPISODE_BATCH_SIZE == (EPISODE_BATCH_SIZE - 1):
                    ...
                    logger.warn('episode %d, average reward %f' % (episode_count, reward_sum / EPISODE_BATCH_SIZE))
                    if reward_sum > 0:
                        logger.warn('Task solved in %d episodes!' % (episode_count))
                    reward_sum = 0

                if episode_count % SAVE_INTERVAL == (SAVE_INTERVAL - 1) and self.name == 'worker_0':
                    logger.warn('save model at epoch %d' % (episode_count))
                    ckpt_file = os.path.join(self.model_path, 'a3c_tennis')
                    saver.save(sess, ckpt_file, (episode_count + 1) / SAVE_INTERVAL)
